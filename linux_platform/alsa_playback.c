...
static bool alsa_volume_driver_init(void);
        ...
        else
        {
            ...
            for (int i = 0; i < ALSA_MIXER_NAME_SIZE; i++){
                ...
                if (snd_mixer_elem != NULL){
                    ...
                    TRACE_LOG("Get Alsa Mixer Vol min: %ld, max: %ld range:%ld", vol_min, vol_max, vol_range);
                    return true;
                }
            }
        }
...
int init_alsa_sink(wiced_bt_a2dp_codec_info_t *codec_info)
{
    ...
    else
    {
        ...
        // Init volume driver
        if (alsa_volume_driver_init() != true){
            TRACE_ERR("alsa_volume_driver_init failed!!!!");
            return ALSA_FAIL;
        }
    }
...
*
****************************************************************************/
int init_alsa_hfp(PLAYBACK_CONFIG_PARAMS pb_config_params)
{
    int status;
    ...
    TRACE_LOG("nblocks %d nchannels %d nsubbands %d ameth %d freq %d format %d latency = %d",
...
bool a2dp_sink_dequeue(){
    ...
    if (pthread_mutex_trylock(&cond_sink_decode_lock) == 0){
        if (!queue_is_empty(sinkQueue)){
            ...
            //TRACE_LOG("frame_len:%d Frame to send:%d, buffer:%d",nodeOut->node_len, alsa_frames_to_send, PcmBytesPerFrame);
            if (pOut == NULL){
                TRACE_ERR ("a2dp_sink_dequeue(): NO BUFFER!");
                 return false;
            }
            else{
