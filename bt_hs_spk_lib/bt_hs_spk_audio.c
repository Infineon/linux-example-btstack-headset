...
bool sinkDecodeStartFlag = FALSE;

wiced_timer_t a2dp_decode_timer;
/**************************************************************************************************
...
static void activeDecodeThread(void);
static void stopDecodeThread(void);
static void decode_timer_cb(WICED_TIMER_PARAM_TYPE arg);
static void delay_decode_by_timer(void);
//=================================================================================================
//  Global Functions
//=================================================================================================
...
wiced_result_t bt_hs_spk_audio_init(bt_hs_spk_control_config_audio_t *p_config, BT_HS_SPK_CONTROL_LOCAL_VOLUME_CHANGE_CB *p_vol_chg_cb)
{
    ...
    wiced_init_timer( &a2dp_decode_timer, &decode_timer_cb, 0, WICED_MILLI_SECONDS_TIMER );
...
void *a2dp_decode_thread(void *args){
    ...
    while(1){
        if (!sinkDecodeStartFlag)
        {
            WICED_BT_TRACE("[%s] WAITING\n",__func__);
            ...
            WICED_BT_TRACE("[%s] ACTIVE\n",__func__);
        }

        //decode
        if (!a2dp_audio_decode()){
            sleep_us(1000);
        }
    }
}
...
/*******************************************************************************
* Function Name: activeDecodeThread 
********************************************************************************
* Summary:
*   *   Start Sink Decode thread by trigger flag and pthread_cond_signal
*
* Parameters:
*   NONE
*
...
*******************************************************************************/
static void activeDecodeThread(void){
    WICED_BT_TRACE("[%s] Decode Start\n",__func__);
    sinkDecodeStartFlag = TRUE;
    pthread_mutex_lock(&cond_decode_lock);
...
/*******************************************************************************
* Function Name: stopDecodeThread 
********************************************************************************
* Summary:
*   Stop Sink Decode thread by trigger flag
*
* Parameters:
*   NONE
*
...
*******************************************************************************/
static void stopDecodeThread(void){
    WICED_BT_TRACE("[%s] Decode Stop\n",__func__);
    sinkDecodeStartFlag = FALSE;
}


static void decode_timer_cb(WICED_TIMER_PARAM_TYPE arg)
{
    wiced_stop_timer( &a2dp_decode_timer );
    activeDecodeThread();
}
...
{
    stopDecodeThread();
    if (!wiced_is_timer_in_use(&a2dp_decode_timer)){
        wiced_start_timer(&a2dp_decode_timer, DELAY_DECODE_MILLISEC);
    }
}
